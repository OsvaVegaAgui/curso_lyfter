...
def print_numbers_times_2(numbers_list):# O(n)
	for number in numbers_list:# O(n)
		print(number * 2)# O(1)
# EL AGORITMO ANTERIOR ES UN O(n), YA QUE ES AHI DONDE ALCANZA SU NIVEL MÁS CRÍTICO
...
def check_if_lists_have_an_equal(list_a, list_b): # O(n^2)
	...
	return False # O(1)
# EL ALGORITMO ANTERIOR ES UN # O(n^2), YA QUE ES AHI DONDE ALCANZA SU NIVEL MÁS CRÍTICO
...
# EL ALGORITMO ANTERIOR ES UN O(1), YA QUE ES AHI DONDE ALCANZA SU NIVEL MÁS CRÍTICO
...
# EL ALGORITMO ANTERIOR ES UN O(n^3), YA QUE ES AHI DONDE ALCANZA SU NIVEL MÁS CRÍTICO
